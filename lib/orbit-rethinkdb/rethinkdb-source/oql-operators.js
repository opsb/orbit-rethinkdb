export default {
  ...
  relatedRecord: {
    evaluate(context, args) {
      ...
      // const tableName = serializer.tableName(model);
      // const fieldName = serializer.fieldName(`relationships/${relationshipName}`);
      // const inverseTableName = serializer.tableName(relationship.model);

      // const chatRoomId = r.table(tableName).get(recordId).getField(fieldName);
      // return r.table(inverseTableName).get(chatRoomId);


      return r.db('orbit_rethinkdb').table('chat_rooms').filter({id: r.db('orbit_rethinkdb').table('messages').get('message1').getField('chatRoomId')});
    },
  },
