  ...
  _replaceHasMany(operation) {
    ...
    if (inverseRelationshipDefinition.type === 'hasOne') {
      updateInverse = this._rethinkdbDao
        ...
        .then(() => this._rethinkdbDao.replaceHasOnes(relationship, ids, inverseRelationship, recordId));
    } else {
      updateInverse = this._rethinkdbDao
        .removeFromHasManysMatching(relationship, inverseRelationship, recordId)
        .then(() => this._rethinkdbDao.addToHasManys(relationship, ids, inverseRelationship, recordId));
    }
